  ...
  return (
    <Sider collapsible collapsed={collapsed} onCollapse={(value) => setCollapsed(value)}>
      <Menu theme="dark" mode="inline" defaultSelectedKeys={['1']}>
        ...
        <Menu.Item key="3" icon={<TableOutlined />} onClick={() => handleMenuClick('/behaviourtable')}>
          Behaviour Table
        </Menu.Item>
        <Menu.Item key="4" icon={<UploadOutlined />} onClick={() => handleMenuClick('/upload')}>
          Upload
        </Menu.Item>
        <Menu.Item key="5" icon={<MessageOutlined />} onClick={() => handleMenuClick('/chat')}>
          Chat
        </Menu.Item>
        {/* 🔥 New Links Added Below */}
        <Menu.Item key="6" icon={<TeamOutlined />} onClick={() => handleMenuClick('/employee-form')}>
          Employee-Form
        </Menu.Item>
